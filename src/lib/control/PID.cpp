...
namespace Lib1104A
{
    namespace Control
    {
        ...
        double PID::calculate(double input)
        {
            ...
            if (m_maxError == std::numeric_limits<double>::max() || std::fabs(m_error) <= m_maxError)
            {
                m_integral += m_error * Dt;
            }
            if (m_integralReset && std::copysign(1.0, m_error) != std::copysign(1.0, m_lastError))
            {
                m_integral = 0.0;
            }
            ...
            return m_gains.m_Kp * m_error + m_gains.m_Ki * m_integral + m_gains.m_Kd * m_derivative + std::copysign(m_gains.m_Kbias, m_error);
        }
